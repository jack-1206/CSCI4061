...
************************************************/
//this is adapted from tcp_server.c
void init(int port) {
  int server_fd, valret;
  ...
  valret = setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
...
int accept_connection(void) {
  ...
  new_socket = accept(master_fd, (struct sockaddr *)&address, (socklen_t*)&address_len);
  if(new_socket < 0){
    perror("accept");
    exit(1);
  }
...
int get_request(int fd, char *filename) {
	char arr[1024];
	int size=read(fd, arr, 1023);
	if(size>=0){
		if(strncmp(arr, "GET ", 4)!=0){
			return -1;
		}
		printf("GET ");
		fflush(stdout);
		int i=4;//loop in arr
		int j=0;//loop in filename
		while((arr[i]!=' ') && (arr[i]!='\n')){
			filename[j]=arr[i];
			j++;
			i++;
